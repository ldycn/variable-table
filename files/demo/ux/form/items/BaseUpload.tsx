...
function BaseUpload(props: BaseUploadProps) {
  ...
  const finalAllowedExtensions =
    allowedExtensions && allowedExtensions.length > 0 ? allowedExtensions : ALLOWED_EXTENSIONS

  useEffect(() => {
    if (value) {
      setFileList([
        { uid: '-1', name: getFileNameFromUrl(value), status: 'done', url: processImageUrl(value) }
      ])
    } else {
      setFileList([])
    }
