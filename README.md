...
1. 本项目基础库采用的是本人上家公司编写的ux基础库组件（部分删减和微调），可能功能存在一些功能冗余，人认为保留部分冗余有助于面试官更快速的理解本人的编码、架构思路，所以没有删除
...
- **页面三层分离**: 每个页面由 `index.tsx`（视图）、`usePage.tsx`（逻辑）、`configs.tsx`（配置）三部分组成。
- **基础库复用**: `src/ux/` 封装了表单、表格、搜索等基础组件，供页面层统一调用。
...
10. 给ai提出细节优化点并审核对应代码和测试用例：
    1. 设置行数据的时候最终通过循环调用setFieldValue而不是setFieldsValue来修改表单值以防止表格定位变动2
    2. 列配置不需要useMemo缓存，直接在configs中声明,之后在index中使用和引入usePageStore中需要的数据和方法即可.
    3. 使用基础库的getSerialNumberColumnConfig方法生成序号列
    4. 删除输入框的长度限制
    5. 删除项目src/中不影响测试用例通过的无用功能的代码，方便其他人阅读项目代码
    6. 在尽可能小的改动中，将选择框打开弹窗的方式从单击变成双击并修改添加对应测试用例
11. 手动测试并提出细节优化点并审核对应代码和测试用例：
    1. int data type下，default value 不应该可以为0001，onBlur时应该保存Number()之后的结果，修改添加对应测试用例
12. 完成
