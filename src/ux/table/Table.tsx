...
const Table = forwardRef(<T,>(props: TableProps<T>, outRef) => {
  ...
  const isMultiRowsHeader = (props.columns as any[]).some(v => v.children)
  const mergedColumns = useMemo(() => {
    if (isMultiRowsHeader) return props.columns
  ...
  }, [props.columns, isMultiRowsHeader, handleResize])

  const dataSource = useMemo(() => {
    if (!props.pagination || !(props.pagination as any).current) {
      return props.dataSource?.map((v, index) => ({ ...v, __serialNumber__: index + 1 }))
    }
    return props.dataSource?.map((v, index) => ({
      ...v,
      __serialNumber__:
        ((props.pagination as any)?.pageSize || 0) * ((props.pagination as any).current - 1) + index + 1
    }))
  ...
  const scrollY = useMemo(() => {
    ...
    return (
      (height || 0) -
      ((HeaderSize[props.size as any] || HeaderSize.small) + (props.pagination !== false ? 56 : 0))
    )
